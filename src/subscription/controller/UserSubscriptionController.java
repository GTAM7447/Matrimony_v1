...
public class UserSubscriptionController {
    ...
    @Operation(
        ...
        description = "Check if user can perform a specific action (PROFILE_VIEW, CONTACT_REVEAL, VIDEO_CALL, etc.)"
    )
    public ResponseEntity<ApiResponse<Boolean>> canPerformAction(@PathVariable String actionType) {
        Integer userId = SecurityUtil.getCurrentUserId();
        log.info("GET /api/subscriptions/can-perform/{} - User {} checking permission", actionType, userId);
        
        boolean canPerform = subscriptionService.canPerformAction(userId, actionType);
